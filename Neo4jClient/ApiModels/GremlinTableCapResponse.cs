...
namespace Neo4jClient.ApiModels
{
    internal class GremlinTableCapResponse
    {
        ...
        internal static IEnumerable<TResult> TransferResponseToResult<TResult>(IEnumerable<GremlinTableCapResponse> response) where TResult : new()
        {
            var type = typeof(TResult);
            var properties = type.GetProperties();

            var gremlinTableCapResponse = response.SingleOrDefault(); //ToDo Added support for multiple table results.
